...
func NewPgEventRepository(db *sql.DB) (domain.EventRepository, error) {
	return &pgEventRepository{db: db}, nil
}

func (repository *pgEventRepository) CreateEvent(event *domain.Event) error {
	query := `
...
func (repository *pgEventRepository) CreateTicket(ticket *domain.Ticket) error {
	query := `
...
func (repository *pgEventRepository) CreateSpot(spot *domain.Spot) error {
	query := `
...
func (repository *pgEventRepository) ReserveSpot(spotId, ticketId string) error {
	query := `
		UPDATE spots
...
func (repository *pgEventRepository) FindEventByID(eventId string) (*domain.Event, error) {
	query := `
		SELECT 
	...
	for rows.Next() {
		var eventIdStr, eventName, eventLocation, eventOrganization, eventRating, eventImageURL sql.NullString
		...
		err := rows.Scan(
			&eventIdStr, &eventName, &eventLocation, &eventOrganization, &eventRating,
			&eventDate, &eventImageURL, &eventCapacity, &eventPrice, &partnerId,
		...
		if !eventIdStr.Valid || !eventName.Valid || !eventLocation.Valid ||
			!eventOrganization.Valid || !eventRating.Valid || !eventDate.Valid ||
			!eventImageURL.Valid || !eventPrice.Valid || !partnerId.Valid {
			continue
		}
...
func (repository *pgEventRepository) FindSpotByName(eventId string, spotName string) (*domain.Spot, error) {
	query := `
		SELECT
			s.id, s.event_id, s.name, s.status, s.ticket_id,
	...
	`
	row := repository.db.QueryRow(query, eventId, spotName)
...
func (repository *pgEventRepository) FindSpotsByEventID(eventId string) ([]*domain.Spot, error) {
	query := `
...
func (repository *pgEventRepository) ListEvents() ([]domain.Event, error) {
	query := `
		SELECT 
			e.id, e.name, e.location, e.organization, e.rating, e.date, e.image_url, e.capacity, e.price, e.partner_id,
			s.id, s.event_id, s.name, s.status, s.ticket_id,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&eventId, &eventName, &eventLocation, &eventOrganization, &eventRating,
			&eventDate, &eventImageURL, &eventCapacity, &eventPrice, &partnerId,
			&spotId, &spotEventId, &spotName, &spotStatus, &spotTicketId,
			&ticketId, &ticketEventId, &ticketSpotId, &ticketType, &ticketPrice,
		)
		...
		if !eventId.Valid || !eventName.Valid || !eventLocation.Valid ||
			!eventOrganization.Valid || !eventRating.Valid || !eventDate.Valid ||
			!eventImageURL.Valid || !eventPrice.Valid || !partnerId.Valid {
			continue
		}
